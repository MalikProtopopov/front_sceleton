...
export const KANBAN_COLUMNS: { status: InquiryStatus; color: string; bgColor: string }[] = [
...
export function KanbanBoard({
  ...
  const customCollisionDetection: CollisionDetection = useCallback(
    (args) => {
      ...
      if (intersections.length > 0) {
        ...
        if (cardIntersection) {
          const targetInquiry = items.find((i) => i.id === cardIntersection.id);
          if (targetInquiry) {
  ...
  const handleDragOver = useCallback((event: DragOverEvent) => {
    ...
    const targetInquiry = items.find((i) => i.id === overId);
    if (targetInquiry) {
  ...
  const handleDragEnd = useCallback(
    (event: DragEndEvent) => {
      ...
      const draggedInquiry = items.find((i) => i.id === active.id);
      ...
      if (newStatus && newStatus !== draggedInquiry.status) {
        onStatusChange(draggedInquiry.id, newStatus);
      }
    },
