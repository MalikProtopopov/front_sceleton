...
import { Settings, Globe, Bell, BarChart3, KeyRound, MessageSquare, Search, Plus, Trash2, ImageIcon, Info, Mail, Send } from "lucide-react";
...
import type { UpdateTenantSettingsDto, SitemapStaticPage, TenantSettings } from "@/entities/tenant";
...
export default function SettingsPage() {
  ...
  const { mutate: sendTestEmail, isPending: isSendingTestEmail } = useSendTestEmail(tenantId);
    ...
    if (tenant) {
      ...
      if (tenant.settings) {
        ...
        setSeoForm({
          ...
          llms_txt_custom_content: tenant.settings.llms_txt_custom_content || "",
        });
        ...
        setEmailForm({
          ...
          smtp_port: tenant.settings.smtp_port != null ? String(tenant.settings.smtp_port) : "",
          smtp_user: tenant.settings.smtp_user || "",
          smtp_use_tls: tenant.settings.smtp_use_tls ?? true,
          smtp_password: "",
          email_api_key: "",
        });
        setSmtpPasswordConfigured(tenant.settings.smtp_password_configured ?? false);
        setEmailApiKeyConfigured(tenant.settings.email_api_key_configured ?? false);
      }
    }
  }, [tenant]);
        ...
        >
          <div className="mt-6 space-y-6">
                ...
                <CardContent className="space-y-4">
                  ...
                  <div>
                    <Input
                      ...
                      placeholder={smtpPasswordConfigured ? "••••••• (настроен, оставьте пустым чтобы не менять)" : "Введите пароль"}
                    />
                    {smtpPasswordConfigured && !emailForm.smtp_password && (
                      <p className="mt-1 text-xs text-green-600">Пароль настроен</p>
                    )}
                  </div>
                  <Switch
            ...
            {(emailForm.email_provider === "sendgrid" || emailForm.email_provider === "mailgun") && (
              <Card>
                <CardHeader>
                  <CardTitle>API-ключ {emailForm.email_provider === "sendgrid" ? "SendGrid" : "Mailgun"}</CardTitle>
                </CardHeader>
                <CardContent className="space-y-4">
                  <div>
                    <Input
                      ...
                      placeholder={emailApiKeyConfigured ? "••••••• (настроен, оставьте пустым чтобы не менять)" : "Введите API-ключ"}
                    />
                    {emailApiKeyConfigured && !emailForm.email_api_key && (
                      <p className="mt-1 text-xs text-green-600">API-ключ настроен</p>
                    )}
                  </div>
                </CardContent>
              </Card>
